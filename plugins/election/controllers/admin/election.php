...
class Election_Controller extends Tools_Controller {
	...
	public function index()
	{
		...
		if ($_POST)
		{
			...
			if ($post->action == 'a')
			{
				...
				$post->add_rules('boundary_id','required');
			}
			...
			if ($post->validate())
			{
				...
				elseif ($post->action == 'a')
				{
					...
					if ($monitor->loaded)
					{
						...
						$monitor->boundary_id = $post->boundary_id;
		...
		$monitors = ORM::factory('monitor')->orderby('monitor.boundary_id', 'asc')
		...
		$this->template->header->js .= View::factory('js/monitor_js');
	}
	...
	public function boundaries()
	{
		...
		(
			...
			// Javascript Header
			$this->template->colorpicker_enabled = TRUE;
			$this->template->header->js .= View::factory('js/boundary_js');
	}
	...
	public function codes()
	{
        ...
        // Javascript Header
        $this->template->colorpicker_enabled = TRUE;
        $this->template->header->js .= View::factory('js/code_js');
	}
	...
	public function admin_sections(){
	   ...
	   // Pagination
        ...
        // Javascript Header
        $this->template->colorpicker_enabled = TRUE;
        $this->template->header->js .= View::factory('js/adminsection_js');


	}

	public function report_sections()
	{
		...
		(
			...
			if ($_POST)
			{
				...
				if ($post->validate())
				{
					...
					{ 
				        ...
				        // reportsections
						if ($post->reportsection_id_action != 1 AND $post->reportsection_id_action !=2)
						{   
			...
			// Javascript Header
			$this->template->colorpicker_enabled = TRUE;
			$this->template->header->js .= View::factory('js/reportsection_js');
	}
