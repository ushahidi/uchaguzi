	...
	public function index()
	{
		...
		if ( $_POST )
		{
			...
			if ($post->validate())
			{
				...
				$this->template->header->js = new View('admin/upgrade/upgrade_status_js');
				$this->template->header->js->backup = $post->chk_db_backup_box;
				...
				// Log file location
				$this->template->header->js->log_file = url::site(). "admin/upgrade/logfile?f=".$this->session->get('upgrade_session').".txt";
			}
			 // No! We have validation errors, we need to show the form again, with the errors
			else
			{
				$this->template->header->js = new View('admin/upgrade/upgrade_js');
